...
function initXMLPlus(xmlType) {
  ...
  eval(`document.create` + xmlType + `Element=function(tag){
      
      document.getElementById('`+ xmlType + `Forge').contentWindow.document.getElementsByTagName('body')[0].innerHTML='<'+tag+'></'+tag+'>';
      let tg=document.getElementById('`+ xmlType + `Forge').contentWindow.document.getElementsByTagName('body')[0].firstElementChild;
      
        document.getElementById('`+ xmlType + `Stage').appendChild(tg);
        return tg;
